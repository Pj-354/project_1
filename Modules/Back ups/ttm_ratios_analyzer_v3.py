...
class TTMRatiosAnalyzer:
    ...
    def _attach_effective_fiscal_year(
        ...
        # Helper: infer fiscal year from a date and a fiscal year-end pattern
        def _infer_fy_from_date(e_norm: pd.Timestamp,
                                fy_end_month: Optional[int],
                                fy_end_day: Optional[int]) -> Optional[int]:
            if fy_end_month is None:
                # No usable pattern
                return None

            if fy_end_day is not None:
                # Stable month + day (e.g. Jan 31, Mar 31, Dec 31)
                if (e_norm.month, e_norm.day) <= (fy_end_month, fy_end_day):
                    return int(e_norm.year)
                else:
                    return int(e_norm.year) + 1
            else:
                # Only month is stable (e.g. 52/53-week year ending any day in January)
                if e_norm.month <= fy_end_month:
                    return int(e_norm.year)
                else:
                    return int(e_norm.year) + 1
        ...
        for ticker, tgrp in df.groupby(ticker_col, dropna=False):
            ...
            if not annual.empty:
                # --- build annual cycles, labelling by calendar year of end_date ---
                for _, row in annual.iterrows():
                    ...
                    if pd.isna(start):
                        if cycles:
                            ...
                            start = end_norm - pd.Timedelta(days=365)

                    cycles.append((start.normalize(), end_norm, fy_label))

                # --- derive fiscal year-end pattern and last_cycle metadata ---
                fy_end_month: Optional[int] = None
                fy_end_day: Optional[int] = None
                last_cycle_end: Optional[pd.Timestamp] = None
                last_cycle_fy: Optional[int] = None

                if cycles:
                    annual_ends = [c_end for _, c_end, _ in cycles]
                    last_cycle_end = cycles[-1][1]
                    last_cycle_fy = cycles[-1][2]

                    end_md = {(d.month, d.day) for d in annual_ends}
                    end_m = {d.month for d in annual_ends}

                    if len(end_md) == 1:
                        # Exact month+day year-end (e.g. Jan 31 every year)
                        fy_end_month, fy_end_day = next(iter(end_md))
                    elif len(end_m) == 1:
                        # 52/53-week pattern: stable month, varying day (e.g. last Sunday in Jan)
                        fy_end_month = next(iter(end_m))
                        fy_end_day = None
                    else:
                        fy_end_month = fy_end_day = None

                # --- assign effective_fiscal_year to all rows for this ticker ---
                for ridx, rrow in tgrp.iterrows():
                    ...
                    # 1) Try to assign based on annual cycles
                    ...
                    if not assigned:
                        ...
                        fy_label: Optional[int] = None

                        # 2) Try to infer from fiscal year-end pattern for periods AFTER last annual
                        fy_inferred: Optional[int] = None
                        if (
                            fy_end_month is not None
                            and last_cycle_end is not None
                            and e_norm > last_cycle_end
                        ):
                            fy_inferred = _infer_fy_from_date(e_norm, fy_end_month, fy_end_day)

                            # Monotonicity check: after the last annual,
                            # inferred FY should be at least last_cycle_fy + 1
                            if fy_inferred is not None and last_cycle_fy is not None:
                                min_next_fy = last_cycle_fy + 1
                                if fy_inferred < min_next_fy:
                                    fy_inferred = min_next_fy

                        # 3) Choose final label: prefer inferred, else tag, else calendar year
                        if fy_inferred is not None:
                            fy_label = fy_inferred
                ...
                # No annual rows at all: fall back to tag or end.year
                for ridx, rrow in tgrp.iterrows():
                    e = rrow[end_col]
                    if pd.isna(e):
                        continue
    ...
    def _classify_period_type_vectorized(
        ...
        """
        Vectorized version of `_classify_period_type` for performance.
        """
        if df.empty:
        ...
        is_fy = form.isin(TTMRatiosAnalyzer._FY_FORMS) | (period_days >= TTMRatiosAnalyzer._FULL_YEAR_MIN_DAYS)
