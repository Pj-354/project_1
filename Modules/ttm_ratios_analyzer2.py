...
DEFAULT_METRIC_COMPONENTS_PATH = PROJECT_ROOT / "data/metric_components.json"
...
class TTMRatiosAnalyzer:
    ...
    debug: bool = False
    register_unrecognized_fields: bool = True
    ...
    metric_components_path: str = "/Users/phillip/Desktop/Moon2/data/metric_components.json"
    ...
    def run_data_quality_checks(self, df: pd.DataFrame, null_threshold: int = 50) -> pd.DataFrame:
        ...
        # 2.2 Total liabilities ≈ current + noncurrent
        if {"bs_total_liabilities", "bs_current_liabilities", "bs_noncurrent_liabilities"}.issubset(df.columns):
            residual = (
                df["bs_total_liabilities"]
                - (df["bs_current_liabilities"] + df["bs_noncurrent_liabilities"])
            )
            ...
            for q, bad, r in zip(quarters, bad_mask, res_arr):
                if bad:
                    add_issue(
                        ...
                        "ACCOUNTING_IDENTITY_MISMATCH",
                        r,
                        details="Total liabilities != current + noncurrent"
                    )
    ...
    def extract_error_context(
        ...
        CONTEXT_MAP = {
            ...
            ("bs_total_liabilities", "ACCOUNTING_IDENTITY_MISMATCH"): [
                "bs_total_liabilities", "bs_current_liabilities", "bs_noncurrent_liabilities"
            ],
        ...
        if not records:
            return pd.DataFrame(columns=["quarter", "metric", "error", "details"])
    ...
    def _attach_effective_fiscal_year(
        ...
        """Derive an effective fiscal year for each row based on annual filings."""
        if fins.empty:
        ...
        annual_like = (
            ...
            | (df["__fiscal_period_norm"].isin({"FY", "Q0"}))
        ...
        for ticker, tgrp in df.groupby(ticker_col, dropna=False):
            ...
            annual = annual.sort_values(end_col)
            cycles: List[Tuple[pd.Timestamp, pd.Timestamp, int]] = []
            ...
            if not annual.empty:
                for _, row in annual.iterrows():
                    ...
                    fy_raw = row.get(fiscal_year_col)
                    try:
                        fy_label = int(fy_raw) if pd.notna(fy_raw) else int(end.year)
                    except Exception:
                        fy_label = int(end.year)
                    ...
                    cycles.append((start.normalize(), end.normalize(), fy_label))
           
                fy_end_month = fy_end_day = None
                last_cycle_end = None
                if cycles:
                    end_md = {(c_end.month, c_end.day) for _, c_end, _ in cycles}
                    if len(end_md) == 1:
                        fy_end_month, fy_end_day = next(iter(end_md))
                        last_cycle_end = cycles[-1][1]
                ...
                for ridx, rrow in tgrp.iterrows():
                    ...
                    e_norm = e.normalize()
                    assigned = False
                    for c_start, c_end, c_fy in cycles:
                        if c_start <= e_norm <= c_end:
                            df.at[ridx, "effective_fiscal_year"] = c_fy
                            assigned = True
                            break

                    if not assigned:
                        fy_raw = rrow.get(fiscal_year_col)
                        fy_label = None                        
                        # infer fiscal year directly from end_date instead of trusting the tag.
                        if (
                            fy_end_month is not None
                            and last_cycle_end is not None
                            and e_norm > last_cycle_end
                        ):
                            # Example: fy_end_month=1, fy_end_day=31 → FY ends on 31 Jan
                            if (e_norm.month, e_norm.day) <= (fy_end_month, fy_end_day):
                                fy_label = e_norm.year
                            else:
                                fy_label = e_norm.year + 1
                        elif pd.notna(fy_raw):
                            try:
                                fy_label = int(fy_raw)
                            except Exception:
                                fy_label = int(e_norm.year)
                        else:
                            fy_label = int(e_norm.year)
                ...
                for ridx, rrow in tgrp.iterrows():
                    ...
                    if pd.notna(fy_raw):
                        try:
                            df.at[ridx, "effective_fiscal_year"] = int(fy_raw)
                        except Exception:
                            df.at[ridx, "effective_fiscal_year"] = int(e.year)
                    else:
                        df.at[ridx, "effective_fiscal_year"] = int(e.year)

        df = df.drop(columns=["__timeframe_norm", "__fiscal_period_norm", "__form_norm"], errors="ignore")
        return df
    ...
    def _infer_fiscal_year_start_cache(
        ...
        """Infer fiscal-year start dates for each (ticker, fiscal year)."""
        if fins.empty:
            return {}
        ...
        for (tick, fy), grp in df.groupby(group_cols, dropna=False):
            ...
            grp = grp.copy()
            annual_grp = grp[grp["__is_annual_like"] & grp[start_col].notna()]
    ...
    def _ensure_quarterly_coverage(
        ...
        """Ensure each fiscal year has one row per quarter, preferring chosen providers."""
        if df.empty:
            return df.copy(), {}

        data = df.copy()
        quarter_alias = {"Q0": "Q4", "FY": "Q4", "4Q": "Q4", "3Q": "Q3", "2Q": "Q2", "1Q": "Q1"}
        ...
        for (ticker, year), group in data.groupby(["ticker", year_col], dropna=False):
            for q_label in ("Q1", "Q2", "Q3", "Q4"):
                ...
                selected_idx.append(subset.index[0])
    ...
    def _fix_share_count_outliers(
        ...
        An outlier is defined as a value that is significantly smaller than both
        the quarter before and the quarter after. If detected, replace with linear
        interpolation between the surrounding quarters.
        
        Parameters:
        -----------
        df : pd.DataFrame
            Quarterised dataframe with period index or with columns like 'effective_fiscal_year' and 'quarter'
        ...
        --------
        pd.DataFrame
            DataFrame with outliers replaced by linear interpolation, preserving original index
        """
        if df.empty:
            return df
        ...
        if df.index.name == 'period' or (len(df) > 0 and isinstance(df.index[0], str) and 'Q' in str(df.index[0])):
            ...
            df_sorted = df.sort_values(['effective_fiscal_year', 'quarter']).reset_index(drop=True)
            use_index = False
            ...
            # Try to infer from index or use as-is
            df_sorted = df.sort_index()
            use_index = True
            index_mapping = {i: idx for i, idx in enumerate(df_sorted.index)}
        ...
        if use_index:
            ...
            # Restore original index from mapping
    ...
    def _build_quarterized_bucket_views(
        ...
        bucket_eff["period_type"] = bucket_eff.apply(
            lambda row: self._classify_period_type(row, fy_cache), axis=1
        )

        cols = list(self._RAW_META_COLUMNS) + [
            c for c in bucket_eff.columns if c in BUCKET_METRIC_MAP.values()
        ]
        if provider == "poly":
            cols = [c for c in cols if c not in {"access_number", "quarter", "form", "accepted_date"}]
        cols = [c for c in cols if c in bucket_eff.columns]
        bucket_eff = bucket_eff[cols]
    ...
    def finalize_quarterised_financials(
        ...
        *,
        ...
        if ("effective_fiscal_year" not in financials_quarterised.columns 
            ...
            financials_quarterised['effective_fiscal_year'] = financials_quarterised.index.str[0:4]
            financials_quarterised['ticker'] = financials_quarterised['period_key'].dropna()[0].split('_')[0]
        ...
        if earnings_ticker:
            ...
            aligned = self._apply_earnings_release_alignment(working_align)
            aligned["ticker"] = aligned["_original_ticker"]
            working = aligned.drop(columns=["_original_ticker"])
        else:
            working = self._apply_earnings_release_alignment(working)

        if (
            "is_net_income_loss" in working.columns
            and "is_basic_average_shares" in working.columns
        ):
            denom = working["is_basic_average_shares"]
            working["eps_basic"] = (working["is_net_income_loss"] / denom).replace(
                [np.inf, -np.inf], np.nan
            ).round(4)

        if (
            "is_net_income_loss" in working.columns
            and "is_diluted_average_shares" in working.columns
        ):
            denom = working["is_diluted_average_shares"]
            working["eps_diluted"] = (
                working["is_net_income_loss"] / denom
        ...
        return is_statement, balance_sheet, cashflow, misc_df, final_financials
...
##### Financial Quarterised #######
def check_ncf(financials_quarterised):
    net_cashflows = (-financials_quarterised['cf_effect_of_exchange_rate_on_cash_and_cash_equivalents'] + financials_quarterised['cf_net_cash_flow'])
    sum_cashflows = financials_quarterised[['cf_net_cash_flow_from_financing_activities', 'cf_net_cash_flow_from_investing_activities', 'cf_net_cash_flow_from_operating_activities']].sum(axis=1)
    print((net_cashflows - sum_cashflows) / 1e9)

def get_rawfields_finnonly(ticker, freq, raw_api, num, keyword):
    if raw_api is None:
        raw_api = TTMRatiosAnalyzer(verbose=False, interactive=False, debug=True)._fetch_finnhub_raw(ticker, freq).get('data')
        return raw_api
    print(str(raw_api[num].get('year')) + 'Q' + str(raw_api[num].get('quarter')))
    raw_api_data = ([i for i in raw_api[num].get('report').get('ic')]
...
##### SEC Functions #########
def _load_SEC_from_parquet(ticker: str, analyzer : TTMRatiosAnalyzer):
    """
    From a saved parquet -> get the parquet and return flow and stock raw dataframe 
    Must pass the output through pivot and select columns
    """
    raw_parquet = pd.read_parquet(f"/Users/phillip/Desktop/Moon2/data/SEC_fundamentals/{ticker}_SEC_raw.parquet")
    # Map GAAP tags -> canonical once
    canon_map = {
    ...
    base = (
        ...
        .loc[lambda d: d["val"].notna()]                    # mimic val is not None
        .assign(canonical=lambda d: d["raw_concept"].map(canon_map))
    )

    # Stock (no start date -> balance sheet)
    stock_raw_df = (
...
def identify_overlapping_alias_mapping_SEC(df):
    df = df.copy()
...
def _pivot_SEC(df: pd.DataFrame) -> pd.DataFrame:
    """ 
    Pivot the wide SEC data we store as parquet to
    long form
    """
    if "start" and "end" in df.columns:
        return df.pivot(
            ...
            columns='canonical',
            values='val').reset_index().rename(columns={'start': 'start_date', 'end': 'end_date', 'fy': 'fiscal_year', 'fp': 'fiscal_period'})
        return df.pivot(
            index=["time", "fy", "fp", "filing_date", "ticker"],
            columns='canonical',
            values='val').reset_index().rename(columns={'time': 'end_date', 'fy': 'fiscal_year', 'fp': 'fiscal_period'}
        )

def _select_cols_from_SEC(long_sec_df: pd.DataFrame, selected_cols: List[str], analyzer : TTMRatiosAnalyzer = None) -> pd.DataFrame:
    """ 
    Matches for either flow or stock SEC data (in long form) columns based on canonical
    """
    if analyzer is None:
    ...
    keep_cols = ['canonical','val', 'fy', 'fp', 'start', 'end', 'filing_date', "ticker"] if 'start' in long_sec_df.columns else ['canonical','val', 'fy', 'fp', 'time', 'filing_date', "ticker"]
    _select_df       = long_sec_df[
        long_sec_df['canonical'].isin(available_cols)][keep_cols]
