    ...
    def finalize_quarterised_financials(
        ...
        *,
        ...
        def _scale_frame(frame: pd.DataFrame) -> pd.DataFrame:
            ...
            skip_contains = ("earnings_per_share", "shares")
            ...
            for col in scaled.columns:
                ...
                if col in skip_exact:
                    continue
                # Consider columns that are numeric or can be coerced to numeric
                ser = pd.to_numeric(scaled[col], errors="coerce")
                if ser.notna().any():
                    scaled[col] = ser / factor
            return scaled
